...
from .models import LeclercProductMatch, LidlProductMatch, PriceComparison
...
@admin.register(LidlProductMatch)
class LidlProductMatchAdmin(admin.ModelAdmin):
    list_display = ('ingredient', 'product_name', 'price', 'store_id', 'is_available', 'last_updated')
    list_filter = ('is_available', 'store_id')
    search_fields = ('ingredient__nom', 'product_name')
    readonly_fields = ('created_at', 'last_updated')
...
class PriceComparisonAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'panier', 'leclerc_total', 'lidl_total', 'cheapest_supermarket', 'created_at')
